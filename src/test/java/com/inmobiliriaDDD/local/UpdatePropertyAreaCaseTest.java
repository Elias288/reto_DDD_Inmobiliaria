...
import com.inmobiliariadomain.local.usecases.UpdatedPropertyAreaUseCase;
...
class UpdatePropertyAreaCaseTest {

    @InjectMocks
    private UpdatedPropertyAreaUseCase useCase;

    @Mock
    private DomainEventRepository repository;

    @Test
    void updatePropertyArea(){
        //arrange
        ManagementID fakeManagementID = ManagementID.of("fakeManagementID");

        PropertyID fakePropertyID = PropertyID.of("fakePropertyID");
        Area updateArea = new Area(40);

        var command = new UpdatePropertyArea(fakeManagementID, fakePropertyID, updateArea);
        ...
        //assert
        var event = (PropertyAreaUpdated) domainEvents.get(0);

        assertEquals("fakePropertyID", event.getPropertyID().value());
        assertEquals(40, event.getArea().value().intValue());
