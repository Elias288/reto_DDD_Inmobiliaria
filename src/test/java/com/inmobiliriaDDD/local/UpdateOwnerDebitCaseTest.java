...
import com.inmobiliariadomain.local.usecases.UpdatedOwnerDebitUseCase;
...
class UpdateOwnerDebitCaseTest {

    @InjectMocks
    private UpdatedOwnerDebitUseCase useCase;

    @Mock
    private DomainEventRepository repository;

    @Test
    void updateOwnerDebit(){
        //arrange
        ManagementID fakeManagementID = ManagementID.of("fakeManagementID");

        OwnerID fakeOwnerID = OwnerID.of("fakeOwnerID");
        Debit updateDebit = new Debit(10000);

        var command = new UpdateOwnerDebit(fakeOwnerID, fakeManagementID, updateDebit);
        ...
        //assert
        var event = (OwnerDebitUpdated) domainEvents.get(0);

        assertEquals("fakeOwnerID", event.getOwnerID().value());
        assertEquals(10000, event.getDebit().value().intValue());
