...
import com.inmobiliariadomain.promotionsection.usecases.UpdatedOfferIsFinishedUseCase;
...
class UpdateOfferIsFinishedCaseTest {

    @InjectMocks
    private UpdatedOfferIsFinishedUseCase useCase;

    @Mock
    private DomainEventRepository repository;

    @Test
    void updateOfferIsFinished(){
        //arrange
        PairingID fakePairingID = PairingID.of("fakePairingID");

        OfferID fakeOfferID = OfferID.of("fakeOfferID");
        IsFinished fakeIsFinished = new IsFinished(true);


        var command = new UpdateOfferIsFinished(fakePairingID, fakeOfferID, fakeIsFinished);
        ...
        //assert
        var event = (OfferIsFinishedUpdated) domainEvents.get(0);

        assertEquals("fakeOfferID", event.getOfferID().value());
        assertEquals(true, event.getIsFinished().value());
