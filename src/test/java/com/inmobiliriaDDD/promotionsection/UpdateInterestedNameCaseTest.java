...
class UpdateInterestedNameCaseTest {

    @InjectMocks
    private UpdatedInterestedNameUseCase useCase;
    ...
    void updateOfferDate(){
        ...
        Name fakeName = new Name("Eddi");


        var command = new UpdateInterestedName(fakePairingID, fakeInterestedID, fakeName);
        ...
        //assert
        var event = (InterestedNameUpdated) domainEvents.get(0);

        assertEquals("fakeInterestedID", event.getInterestedID().value());
        assertEquals("Eddi", event.getName().value());
