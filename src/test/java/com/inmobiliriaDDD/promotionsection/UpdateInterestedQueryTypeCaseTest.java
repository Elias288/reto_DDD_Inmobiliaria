...
import com.inmobiliariadomain.promotionsection.usecases.UpdatedInterestedQueryTypeUseCase;
...
class UpdateInterestedQueryTypeCaseTest {

    @InjectMocks
    private UpdatedInterestedQueryTypeUseCase useCase;
    ...
    void updateOfferDate(){
        ...
        QueryType fakeQueryType = new QueryType(QueryTypeEnum.SALE);


        var command = new UpdateInterestedQueryType(fakePairingID, fakeInterestedID, fakeQueryType);
        ...
        //assert
        var event = (InterestedQueryTypeUpdated) domainEvents.get(0);

        assertEquals("fakeInterestedID", event.getInterestedID().value());
        assertEquals(QueryTypeEnum.SALE, event.getQueryType().value());
